...
if __name__ == "__main__":
    ...
    data = pd.read_csv(data_path/"ratings_meta_Movies_and_TV_valid_random_08_train.csv").iloc[:, 1:] #add the .iloc[:, 1:] to drop first column
    split_location = int(np.floor(ratio*len(data)))
    data = data.iloc[:split_location]
    val_data = pd.read_csv(data_path/"ratings_meta_Movies_and_TV_valid_random_01_val.csv").iloc[:, 1:] #add the .iloc[:, 1:] to drop first column
    test_data = pd.read_csv(data_path/"ratings_meta_Movies_and_TV_valid_random_01_test.csv").iloc[:, 1:] #add the .iloc[:, 1:] to drop first column
