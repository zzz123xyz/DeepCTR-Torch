...
if __name__ == "__main__":
    ...
    data = pd.read_csv("../../dataset/ml-1m/ratings_movies_poster_users_train_binary.csv").iloc[:, 1:] #add the .iloc[:, 1:] to drop first column
    split_location = int(np.floor(ratio*len(data)))
    data = data.iloc[:split_location]
    val_data = pd.read_csv("../../dataset/ml-1m/ratings_movies_poster_users_val_binary.csv").iloc[:, 1:] #add the .iloc[:, 1:] to drop first column
    test_data = pd.read_csv("../../dataset/ml-1m/ratings_movies_poster_users_test_binary.csv").iloc[:, 1:] #add the .iloc[:, 1:] to drop first column
